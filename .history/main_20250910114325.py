...
if median_withdrawal_g is not None:
    ...
    st.dataframe(summary_g_fmt, use_container_width=True)
    st.markdown(
        f"By spending what you wanted (${thinking_spend:,.0f}) instead of being a bit more frugal and spending (${whatif_spend:,.0f}), you cost yourself ${total_income_g:,.0f}."
    )
else:
...
if median_withdrawal_s is not None:
    ...
    st.dataframe(summary_s_fmt, use_container_width=True)
    st.markdown(
        f"By spending what you wanted (${thinking_spend:,.0f}) instead of being a bit more frugal and spending (${whatif_spend:,.0f}), you cost yourself ${total_income_s:,.0f}."
    )
else:
