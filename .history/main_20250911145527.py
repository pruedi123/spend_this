...
try:
    ...
    try:
        if 'grand_df' in locals() and isinstance(grand_df, pd.DataFrame) and not grand_df.empty:
            # Prefer the Grand Total columns if present
            ann_col = next((c for c in grand_df.columns if c.startswith('Annual Retirement Income — Grand Total')), None)
            tot_col = next((c for c in grand_df.columns if c.startswith('Total — Grand Total')), None)
            if ann_col and tot_col:
                for _, r in grand_df.iterrows():
                    p = str(r.get("Portfolio","")).strip()
                    summary_lines.append(f"- **{p} (Grand Total):** annual ≈ {_fmt(r[ann_col])}; total ≈ {_fmt(r[tot_col])}.")
            else:
                # Fallback: summarize the columns that exist
                for _, r in grand_df.iterrows():
                    p = str(r.get("Portfolio","")).strip()
                    pieces = []
                    for c in grand_df.columns:
                        if c in ("Portfolio","Years"):
                            continue
                        pieces.append(f"{c}: {_fmt(r[c])}")
                    if pieces:
                        summary_lines.append(f"- **{p}:** " + "; ".join(pieces))
    except Exception:
        pass
