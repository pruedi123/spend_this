...
#
# ==============================================
# Opportunity Cost — Annual Habit Only
# ==============================================
rows_annual = []
raw_rows_annual = []
for alloc in common_allocs:
...
if not result_annual_df.empty:
    ...
    st.subheader("Opportunity Cost — Annual Habit Only (Min & Median by Allocation)")
    st.caption(
        f"Annual investment = (${daily_spend:,.2f} − ${frugal_daily:,.2f}) × {int(days_per_week)} × {int(weeks_per_year)} "
    ...
    st.download_button(
        "Download annual habit table (CSV)",
        data=raw_annual_df.to_csv(index=False).encode("utf-8"),
        file_name=f"spend_this_annual_only_{years}y.csv",
        mime="text/csv"
    )
else:
    st.info("No results to display for the annual habit section.")
...
if median_withdrawal_g is not None:
    ...
    lump_rows.append({
        ...
        "Annual Income (Historically)": ann_g,
...
if median_withdrawal_s is not None:
    ...
    lump_rows.append({
        "Portfolio": "SPX",
        "Years": int(retirement_years),
        "Annual Income (Historically)": ann_s,
        "Total Median Retirement Income": ann_s * int(retirement_years),
    })
if lump_rows:
    lump_df = pd.DataFrame(lump_rows)
    lump_disp = lump_df.copy()
    lump_disp["Annual Income (Historically)"] = lump_disp["Annual Income (Historically)"].map(lambda v: f"${v:,.0f}")
...
# ---------- Annual Habit Only median withdrawal table ----------
annual_rows = []
if median_withdrawal_g is not None and len(raw_rows_annual) > 0:
    ...
    annual_rows.append({
        "Portfolio": "Global",
        "Years": int(retirement_years),
        "Annual Income (Historically)": ann_g_a,
...
if annual_rows:
    ...
    st.download_button(
        "Download median withdrawal — Annual Habit Only (CSV)",
        data=annual_df.to_csv(index=False).encode("utf-8"),
        file_name=f"median_withdrawal_annual_only_{years}y.csv",
        mime="text/csv"
    )
else:
    st.info("Median withdrawals not found for Annual Habit scenario.")
def _lookup_median_withdrawal(df_w: pd.DataFrame | None, yrs: int):
    if df_w is None:
        return None
    try:
