...
if years > 0 and (float(non_price) > 0 or float(frugal_price) > 0):
    ...
    try:
        ...
        # Auto-detect the increase column (deflator) robustly
        _cols = [c for c in _df_inc.columns]
        _col_inc = None
        for c in _cols:
            ...
            if "increase" in s and "factor" in s:
                ...
                break
        if _col_inc is None:
            _col_inc = "increase_factor"  # fallback name
        inc_series = _pd.to_numeric(_df_inc[_col_inc], errors="coerce")
        # IMPORTANT: do NOT dropna here; keep alignment with original indexing
    ...
    def _fv_series_for(df_windows: pd.DataFrame) -> np.ndarray:
        ...
        for start_idx, window in zip(df_windows["start_index"].values.astype(int), df_windows["factors"].values):
            ...
            if np.allclose(levs, 1.0):
                st.caption(f"[debug] CPI levels flat for window start {start_idx} (sheet={_inc_sheet}).")
