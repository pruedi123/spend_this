...
# Annual expense in basis points (like mc_audit)
exp_bps = st.slider("Annual Expense (basis points)", 0, 100, 0, 5)
exp_rate = float(exp_bps) / 10000.0
st.caption(f"Expense applied each compounding step: {exp_bps} bps ({exp_rate:.2%}) per year")
# Our window step is in months (default 12). Fee is applied per step as (1 - e)^(step/12).
_default_step = 12
fee_mult_per_step = (1.0 - exp_rate) ** (_default_step / 12.0)
...
for alloc in common_allocs:
    # Global
    sims_g = build_windows(df_glob, alloc, years, step=12, fee_mult_per_step=fee_mult_per_step)
    if sims_g.empty:
        g_min = g_med = None
    else:
    ...
    # SPX
    sims_s = build_windows(df_spx, alloc, years, step=12, fee_mult_per_step=fee_mult_per_step)
    if sims_s.empty:
        s_min = s_med = None
    else:
