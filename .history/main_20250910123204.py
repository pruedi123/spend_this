...
with st.sidebar:
    ...
    # Show daily vs frugal and annualized contribution
    daily_diff = max(0.0, daily_spend - frugal_daily)
    annual_contrib_disp = max(0.0, daily_diff * float(days_per_week) * float(weeks_per_year))
    st.caption(f"**Daily difference:** ${daily_diff:,.2f}/day")
    st.caption(f"**Annual investable amount:** ${annual_contrib_disp:,.0f}/yr")
