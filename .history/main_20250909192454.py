...
with c1:
    st.subheader(f"Global (LBM){' — overlapped' if align_overlap else ''}")
    chart_g = alt.Chart(dist_glob).mark_line().encode(
        x=alt.X("start_date:T", title="Start Date", scale=alt.Scale(domain=[pd.to_datetime(df_glob["date"]).min(), pd.to_datetime(df_glob["date"]).max()])),
...
with c2:
    st.subheader(f"SP500 (SPX){' — overlapped' if align_overlap else ''}")
    chart_s = alt.Chart(dist_spx).mark_line().encode(
        x=alt.X("start_date:T", title="Start Date", scale=alt.Scale(domain=[pd.to_datetime(df_spx["date"]).min(), pd.to_datetime(df_spx["date"]).max()])),
        y=alt.Y("fv:Q", title="FV of $1 @ 30 years (×)"),
        tooltip=["start_date","fv"]
    ).properties(height=300)
