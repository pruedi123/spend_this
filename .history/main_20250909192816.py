...
def _pick_date_col(df: pd.DataFrame) -> str | None:
    # Strict: require a 'Begin Month' style column
    for c in df.columns:
...
def _normalize(df: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    if date_col is None:
        raise ValueError("Expected a 'Begin Month' column in the CSV. Please add 'Begin Month' and try again.")
...
with st.sidebar:
    ...
    align_overlap = st.checkbox("Align to overlapping 30-year range", value=True, help="If off, each chart starts at its own first valid 30-year window.")
...
st.caption(f"Earliest Begin Month — Global: {pd.to_datetime(df_glob['date']).min().date()} | SPX: {pd.to_datetime(df_spx['date']).min().date()}")
...
dist_glob["ending_value"] = dist_glob["fv"] * 10000
dist_spx["ending_value"] = dist_spx["fv"] * 10000

# Diagnostics: earliest 30-year start for each
earliest_glob = dist_glob["start_date"].min() if not dist_glob.empty else None
earliest_spx  = dist_spx["start_date"].min()  if not dist_spx.empty else None
st.caption(f"First full 30-year start — Global: {earliest_glob.date() if earliest_glob is not None else 'None'}; SPX: {earliest_spx.date() if earliest_spx is not None else 'None'}")
...
if align_overlap:
    ...
    merged = pd.merge(dist_glob.rename(columns={"fv":"fv_global"}),
                      dist_spx.rename(columns={"fv":"fv_spx"}),
                      on="start_date", how="outer").sort_values("start_date").reset_index(drop=True)
merged["delta_spx_minus_global"] = merged["fv_spx"] - merged["fv_global"]

merged["ending_value_global"] = merged["fv_global"] * 10000
merged["ending_value_spx"] = merged["fv_spx"] * 10000

# Metrics
col1, col2, col3 = st.columns(3)
col1.metric("Periods (overlap)", f"{len(merged):,}")
col2.metric("Median Ending Value — Global", f"${np.median(merged['ending_value_global']):,.0f}")
col3.metric("Median Ending Value — SPX", f"${np.median(merged['ending_value_spx']):,.0f}")

# Charts
c1, c2 = st.columns(2)

with c1:
    st.subheader(f"Distribution of Ending Values (Global){' — overlapped' if align_overlap else ''}")
    chart_g = alt.Chart(dist_glob).mark_bar().encode(
        x=alt.X("ending_value:Q", bin=alt.Bin(maxbins=40), title="Ending Value ($)"),
        y=alt.Y("count()", title="Frequency"),
        tooltip=["ending_value"]
    ).properties(height=300)
    st.altair_chart(chart_g, use_container_width=True)

with c2:
    st.subheader(f"Distribution of Ending Values (SPX){' — overlapped' if align_overlap else ''}")
    chart_s = alt.Chart(dist_spx).mark_bar().encode(
        x=alt.X("ending_value:Q", bin=alt.Bin(maxbins=40), title="Ending Value ($)"),
        y=alt.Y("count()", title="Frequency"),
        tooltip=["ending_value"]
    ).properties(height=300)
...
st.dataframe(merged[["start_date", "ending_value_global", "ending_value_spx", "delta_spx_minus_global"]], use_container_width=True)

st.download_button(
    "Download Combined CSV",
    data=merged[["start_date", "ending_value_global", "ending_value_spx", "delta_spx_minus_global"]].to_csv(index=False).encode("utf-8"),
    file_name=f"30y_ending_values_{alloc}_global_vs_spx.csv",
    mime="text/csv",
)
...
with st.expander("Data check (min dates & coverage)"):
    ...
    st.write("First full 30-year start — Global:", earliest_glob.date() if earliest_glob is not None else "None",
             "| SPX:", earliest_spx.date() if earliest_spx is not None else "None")
