...
if median_withdrawal_g is not None:
    ...
    st.markdown(
        f"<div style='white-space: normal; word-break: normal; overflow-wrap: break-word;'>{_msg_g}</div>",
...
if median_withdrawal_s is not None:
    ...
    st.markdown(
        f"<div style='white-space: normal; word-break: normal; overflow-wrap: break-word;'>{_msg_s}</div>",
        unsafe_allow_html=True
    )
else:
