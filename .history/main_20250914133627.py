...
def _reset_all_inputs():
    ...
    # Annual Habits — set count to 0 and clear per-habit fields (supports up to 5 habits)
    st.session_state["Number of habits"] = 0
    for i in range(1, 6):
        for k in (f"daily_spend_{i}", f"frugal_daily_{i}", f"days_per_week_{i}", f"weeks_per_year_{i}"):
            if k in st.session_state:
                st.session_state.pop(k, None)

    # Auto — zero amounts; keep sliders within valid bounds
    ...
    st.session_state["Finance rate (APR %)"] = 0.0
    ...
    # Housing — zero prices/rates; keep valid term
    ...
    # Optional: revert master view to default by clearing its state before rerun
    if "master_view" in st.session_state:
        st.session_state.pop("master_view", None)

    # Refresh with cleared values
    st.rerun()
