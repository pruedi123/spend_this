...
# Try primary types; fall back to temporary types in factors.py if needed
try:
    from core.types import AppConfig
except Exception:
    from factors import AppConfig  # type: ignore
from app.toggles import init_toggle, show_details, render_radio_below_summary
from app.ui_components import add_info_card_css, card
from scenarios import lump
...
# ---------------------------
# Modular config & scenario (stub wiring)
# ---------------------------
app_config = AppConfig(
    years_to_retire=int(years),
    retirement_years=int(retirement_years),
    spend_thinking=float(thinking_spend),
    spend_frugal=float(whatif_spend),
    habits=habits if 'habits' in locals() else [],
    auto={
        "frugal_price": float(frugal_price),
        "frugal_replace": int(frugal_replace),
        "frugal_down": float(frugal_down),
        "frugal_rate": float(frugal_rate),
        "frugal_term": int(frugal_term),
        "apply_residual_dp": bool(apply_residual_dp),
        "non_price": float(non_price),
        "non_down": float(non_down),
        "non_rate": float(non_rate),
        "non_term": int(non_term),
        "non_replace": int(non_replace),
        "dep_y1": int(dep_y1),
        "dep_y2_5": int(dep_y2_5),
        "dep_y6_10": int(dep_y6_10),
        "dep_y11p": int(dep_y11p),
    },
    housing={
        "spender_price": float(house_spender_price),
        "frugal_price": float(house_frugal_price),
        "down_pct": int(house_down_pct),
        "spender_down_amt": float(house_spender_down_amt),
        "frugal_down_amt": float(house_frugal_down_amt),
        "apr": float(house_apr),
        "term": int(house_term),
        "tax_rate": float(house_tax_rate),
    },
)

# Stub helpers dict for scenarios (we will fill these as we migrate logic)
_helpers = {}

# Compute lump scenario via module (returns empty stub for now)
res_lump = lump.compute(app_config, df_glob, df_spx, common_allocs, _helpers)
...
# ---- Modular details toggle (below summary)
render_radio_below_summary()
if show_details():
    # Render modular lump details (currently stub); we will migrate other scenarios next
    lump.render_details(res_lump)