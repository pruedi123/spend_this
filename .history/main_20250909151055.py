...
def _normalize_factors_df(df: pd.DataFrame, source_label: str):
    ...
    # drop rows with NaNs in any allocation
    alloc_cols = [c for c in out.columns if c != 'date']
    out = out.dropna(subset=alloc_cols, how='any').sort_values('date').reset_index(drop=True)
...
def build_car_purchase_events(
    ...
    """
    Return a DataFrame of purchase events for a single pattern.
    Each event includes:
      - month (int)
      - new_price
      - tax_fees
    ...
    Assumes buying NEW at t=0, then every 'interval_years'.
    """
    events = []
    prev_purchase_price = None
    prev_purchase_month = None
    ...
    for y in range(0, horizon_years + 1, interval_years):
        ...
        if prev_purchase_price is None:
            ...
            years_held = max((m - prev_purchase_month) // 12, 0)
...
def car_opportunity_cost(
    ...
    """Compute FV-at-horizon of the car purchase net outflows given return factors."""
    months = horizon_years * 12
    f = factors_window(df_factors, alloc, start_idx, months)
    payments = np.zeros(months)
    for _, row in events.iterrows():
        m = int(row["month"])  # month offset from start
        if 0 <= m < months:
            payments[m] += float(row["net_outflow"])
...
def rolling_opportunity_values(
    df_factors: pd.DataFrame,
    alloc: str,
    horizon_years: int,
    ...
    payments_schedule_builder(months: int) -> np.ndarray returns payments array for that horizon.
    """
    months = horizon_years * 12
    arr = df_factors[alloc].values.astype(float)
    max_start = len(arr) - months
    results = []
    if max_start <= 0:
    ...
    for s in range(0, max_start):
        f = arr[s:s + months]
        payments = payments_schedule_builder(months)
...
with st.sidebar:
    ...
    if mode == "Single start":
        max_start = len(factors) - horizon_years * 12 - 1
        if max_start < 0:
            st.error("Not enough history for that horizon.")
            st.stop()
        idx = st.slider("Start index (0 = earliest)", 0, max_start, 0)
...
with tab1:
    ...
    with colA:
        ...
        one_time_month = st.number_input("When? (months from start)", min_value=0, value=0, step=1)
    with colB:
        monthly_amount = st.number_input("Monthly habit ($/mo)", min_value=0.0, value=0.0, step=10.0, format="%.2f")
        monthly_duration = st.number_input("Monthly duration (months, 0 = through horizon)", min_value=0, value=0, step=1)
    with colC:
    ...
    if run_btn:
        ...
        def build_payments(M: int) -> np.ndarray:
            p = np.zeros(M)
            if one_time_amount > 0:
                p += schedule_one_time(M, int(one_time_month), float(one_time_amount))
            if monthly_amount > 0:
                dur = None if monthly_duration == 0 else int(monthly_duration)
                p += schedule_monthly(M, float(monthly_amount), 0, dur)
            if annual_amount > 0:
                yrs = None if annual_years == 0 else int(annual_years)
                p += schedule_annual(M, float(annual_amount), 0, yrs)
            return p

        if mode == "Single start":
            f = factors_window(factors, alloc, idx, months)
            payments = build_payments(months)
            fv = future_value_of_payments(payments, f)
            ...
            # Show payment schedule and simple chart
            pay_df = pd.DataFrame({"month": np.arange(months), "amount": payments})
            st.dataframe(pay_df[pay_df["amount"] > 0].head(240), use_container_width=True)
            chart = alt.Chart(pay_df[pay_df["amount"] > 0]).mark_bar().encode(
                x=alt.X("month:Q"), y=alt.Y("amount:Q"), tooltip=["month", "amount"]
            ).properties(height=240)
            st.altair_chart(chart, use_container_width=True)

            # download
            csv_bytes = pay_df.to_csv(index=False).encode("utf-8")
            st.download_button("Download payment schedule (CSV)", data=csv_bytes, file_name="spend_schedule.csv", mime="text/csv")

        else:
            dist_df = rolling_opportunity_values(factors, alloc, horizon_years, build_payments)
            if len(dist_df) == 0:
                ...
                st.download_button("Download distribution (CSV)", data=csv_bytes, file_name="opportunity_cost_distribution.csv", mime="text/csv")
    ...
    if run_cars:
        try:
            ...
            if mode == "Single start":
                ...
                # Distribution across all starts
                months_arr = factors[alloc].values.astype(float)
                max_start = len(months_arr) - months
                fvs_frugal = []
                fvs_freq = []
                dates = []
                for s in range(0, max_start):
                    f_slice = months_arr[s:s + months]

                    pay_fru = np.zeros(months)
                    for _, r in frugal_events.iterrows():
                        m = int(r["month"])
                        if 0 <= m < months:
                            pay_fru[m] += float(r["net_outflow"])

                    pay_freq = np.zeros(months)
                    for _, r in frequent_events.iterrows():
                        m = int(r["month"])
                        if 0 <= m < months:
                            pay_freq[m] += float(r["net_outflow"])
                ...
                st.download_button("Download car comparison distribution (CSV)", data=csv_bytes, file_name="car_comparison_distribution.csv", mime="text/csv")
...
st.caption(
    ...
    "Columns should be one 'date' column plus allocation columns (e.g., LBM 100E ... LBM 10E, LBM 100F, or spx60e). "
    "Opportunity cost is computed as the future value at your chosen horizon."
)
