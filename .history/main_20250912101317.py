...
try:
    ...
    with c1:
        st.markdown("**Spend This**")
        st.markdown(_fmt(spend_this_val))
    with c2:
        st.markdown("**Not This**")
        st.markdown(_fmt(not_this_val))
    with c3:
        st.markdown("**Lifetime Retirement Income**")
        st.markdown(_fmt(total_global if (total_global is not None and np.isfinite(total_global)) else 0))
        st.caption("Global Strategy")
    with c4:
        st.markdown("**Lifetime Retirement Income**")
        st.markdown(_fmt(total_spx if (total_spx is not None and np.isfinite(total_spx)) else 0))
        st.caption("SP500 Strategy")
