...
# =============================================================================
# Helpers for earliest dataset date and valid start index
# =============================================================================

def earliest_dataset_date(df: pd.DataFrame) -> pd.Timestamp:
    return pd.to_datetime(df["date"]).min()

def first_valid_start_index_annual(df: pd.DataFrame, alloc: str, years: int, step_months: int = 12) -> int | None:
    arr = df[alloc].values.astype(float)
    max_start = len(arr) - years * step_months
    if max_start < 0:
        return None
    for s in range(0, max_start + 1):
        idxs = s + np.arange(years) * step_months
        if idxs[-1] >= len(arr):
            break
        window = arr[idxs]
...
with st.sidebar:
    ...
    years = 30  # pinned horizon (years)
    st.caption("Horizon fixed at 30 years.")
...
with tab1:
    ...
    # UI assumptions section
    st.subheader("Assumptions")
    st.write("• One-time spend of **$10,000** at **year 0** (start of window).")
    st.write("• Horizon: **30 years** (fixed).")
    st.write("• Factors are **annual (12-month step)**; we use entries at t, t+12, t+24, ...")
    ...
    if run_btn:
        ...
        if mode == "Single start":
            ...
            # Show absolute calendar window for chosen start and horizon
            start_calendar = pd.to_datetime(factors.loc[idx, "date"])
            end_idx = idx + horizon_years * 12
            end_calendar = pd.to_datetime(factors.loc[min(end_idx, len(factors)-1), "date"])
            st.caption(f"Window: {start_calendar.date()} → {end_calendar.date()} ({horizon_years} years)")
            ...
            if len(dist_df) == 0:
                ...
                # Compute chart x-domain and show caption for first valid start
                min_date_all = earliest_dataset_date(factors)
                max_date_all = pd.to_datetime(factors["date"]).max()
                first_valid_idx = first_valid_start_index_annual(factors, alloc, horizon_years, step_months=12)
                first_valid_date = factors.loc[first_valid_idx, "date"] if first_valid_idx is not None else None
                if first_valid_date is not None and pd.to_datetime(first_valid_date) > pd.to_datetime(min_date_all):
                    st.caption(f"First valid {horizon_years}-year window for {alloc}: {pd.to_datetime(first_valid_date).date()} (axis still anchored at dataset start {pd.to_datetime(min_date_all).date()}).")

                dist_chart = alt.Chart(dist_df).mark_line().encode(
                    x=alt.X("start_date:T", title="Start Date", scale=alt.Scale(domain=[pd.to_datetime(min_date_all), pd.to_datetime(max_date_all)])),
                    y=alt.Y("fv:Q", title="FV @ 30 years (real $)"),
                ...
                st.download_button("Download distribution (CSV)", data=csv_bytes, file_name=f"one_spend_10000_30y_{alloc}.csv", mime="text/csv")
