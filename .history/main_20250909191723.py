...
def distribution_fv(df: pd.DataFrame, alloc: str, years: int, amount: float, allow_partial: bool = False) -> pd.DataFrame:
    step = 12
    arr = df[alloc].values.astype(float)
    ...
    for s in range(0, max_start + 1 if not allow_partial else len(arr)):
        if not allow_partial:
            # strict 30-year windows only
            try:
                f = annual_window(df, alloc, s, years, step)
            except Exception:
                continue
            fv = fv_of_lump_sum(amount, f)
            rows.append((df.loc[s, "date"], fv, years))
        else:
            # allow partial: take as many consecutive annual steps as available from s
            # build idxs until we hit end or a NaN
            idxs = []
            k = 0
            while True:
                idx = s + k * step
            ...
            if len(idxs) == 0:
                continue
            f = arr[idxs].astype(float)
            fv = fv_of_lump_sum(amount, f)
            rows.append((df.loc[s, "date"], fv, len(idxs)))
    out = pd.DataFrame(rows, columns=["start_date", "fv", "years_used"]).dropna()
    return out
...
with st.sidebar:
    ...
    YEARS = 30
    allow_partial = st.checkbox("Allow partial window at start (show earlier dates)", value=False, help="If on, computes FV for shorter windows until a full 30-year window is possible.")
    st.caption("Horizon fixed at 30 years (annual 12‑month steps).")

# Compute immediately (no button) for simplicity
try:
    # Show earliest and first few begin months in dataset
    st.write("Earliest begin month in dataset:", pd.to_datetime(factors["date"]).min().date())
    st.write("First 10 begin months:", pd.to_datetime(factors["date"]).head(10).dt.date.tolist())
    dist = distribution_fv(factors, alloc, YEARS, float(amount), allow_partial=allow_partial)
    if dist.empty:
        ...
        if "years_used" in dist.columns and allow_partial:
            full = dist[dist["years_used"] == YEARS]
            if not full.empty:
                p10, p50, p90 = np.percentile(full["fv"], [10, 50, 90])
            st.caption(f"Showing {len(full):,} full 30-year windows; {len(dist) - len(full):,} partial windows included in chart/table.")
        ...
        enc_tooltip = ["start_date", "fv"] + (["years_used"] if "years_used" in dist.columns else [])
        ...
        with st.expander("Data check (dates & coverage)", expanded=False):
            ...
            if fv_start is not None:
                # compute end date for 30-year window
                start_idx = factors.index[factors["date"] == fv_start][0]
                end_idx = start_idx + YEARS * 12 - 1
                if end_idx < len(factors):
                    fv_end = factors.loc[end_idx, "date"]
                    st.write(f"First full {YEARS}-year window: {pd.to_datetime(fv_start).date()} → {pd.to_datetime(fv_end).date()} ({YEARS} factors)")

        # All simulations table (with multiples)
        dist_table = dist.copy()
        if "years_used" in dist_table.columns:
            dist_table["multiple"] = dist_table["fv"] / float(amount)
            cols = ["start_date", "years_used", "fv", "multiple"]
            dist_table = dist_table[cols]
        else:
            dist_table["multiple"] = dist_table["fv"] / float(amount)
