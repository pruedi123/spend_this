...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("Annual Habit (Recurring Savings)")
    daily_spend = st.number_input("Daily Spend ($)", min_value=0.0, value=8.0, step=0.5)
    frugal_daily = st.number_input("Frugal Alternative ($)", min_value=0.0, value=1.0, step=0.5)
    days_per_week = st.slider("Days per Week", 1, 7, 5)
    weeks_per_year = st.slider("Weeks per Year", 1, 52, 52)
...
# Annual contribution from habit change
annual_contrib = max(0.0, (daily_spend - frugal_daily) * float(days_per_week) * float(weeks_per_year))
...
# ==============================================
# Opportunity Cost — Lump Sum + Annual Habit
# ==============================================
rows_habit = []
raw_rows_habit = []
for alloc in common_allocs:
    ...
    if sims_g.empty:
        ...
        # Lump sum value for each window
        fv_lump_g = sims_g["fv_multiple"].values * spend_diff
        # Annuity FV for each window using the per-year factor window
        ann_g = np.array([annuity_fv_from_window(w, annual_contrib, timing="end") for w in sims_g["factors"].values])
        end_g = fv_lump_g + ann_g
        g_min = float(np.nanmin(end_g))
        g_med = float(np.nanmedian(end_g))
    # SPX
    sims_s = build_windows(df_spx, alloc, years, step=12, fee_mult_per_step=fee_mult_per_step_spx)
    if sims_s.empty:
        s_min = s_med = np.nan
    else:
        fv_lump_s = sims_s["fv_multiple"].values * spend_diff
        ann_s = np.array([annuity_fv_from_window(w, annual_contrib, timing="end") for w in sims_s["factors"].values])
        end_s = fv_lump_s + ann_s
        s_min = float(np.nanmin(end_s))
        s_med = float(np.nanmedian(end_s))
    rows_habit.append({
        "Allocation": alloc,
...
result_habit_df = pd.DataFrame(rows_habit)
if not result_habit_df.empty:
    result_habit_df = result_habit_df[[
        "Allocation",
        "Global Minimum Ending Value",
        "SPX Mininimum Ending Value",
        "Global Median Ending Value",
    ...
    st.subheader("Opportunity Cost — Lump Sum + Annual Habit (Min & Median by Allocation)")
    st.caption(
        f"Adds an annual investment equal to (${daily_spend:,.2f} − ${frugal_daily:,.2f}) × {int(days_per_week)} × {int(weeks_per_year)} "
        f"= **${annual_contrib:,.0f} per year**, net of the same expenses (Global 20 bps; SPX 5 bps)."
    )
    st.dataframe(result_habit_df, use_container_width=True)

    raw_habit_df = pd.DataFrame(raw_rows_habit)[[
        "Allocation",
        "Global Minimum Ending Value",
        "SPX Mininimum Ending Value",
        "Global Median Ending Value",
    ...
    st.download_button(
        "Download lump sum + annual habit table (CSV)",
        data=raw_habit_df.to_csv(index=False).encode("utf-8"),
        file_name=f"spend_this_lumpsum_plus_habit_{years}y.csv",
        mime="text/csv"
    )
else:
    st.info("No results to display for the lump sum + annual habit section.")

# ----------------------------------------------
# Combined — Min & Median by Allocation
# ----------------------------------------------
try:
    combined_raw = raw_df.merge(pd.DataFrame(raw_rows_habit), on="Allocation", how="inner",
                                suffixes=(" — Lump Sum", " — Lump + Habit"))
    combined_raw = combined_raw[[
        "Allocation",
        "Global Minimum Ending Value — Lump Sum",
        "SPX Mininimum Ending Value — Lump Sum",
        "Global Median Ending Value — Lump Sum",
        "SPX Median Ending Value — Lump Sum",
        "Global Minimum Ending Value — Lump + Habit",
        "SPX Mininimum Ending Value — Lump + Habit",
        "Global Median Ending Value — Lump + Habit",
        "SPX Median Ending Value — Lump + Habit",
    ]]
    combined_display = combined_raw.copy()
    for col in combined_display.columns:
        if col != "Allocation":
            combined_display[col] = combined_display[col].map(lambda v: f"${v:,.0f}")
    st.subheader("Combined — Min & Median by Allocation (Lump Sum vs Lump + Annual Habit)")
    st.dataframe(combined_display, use_container_width=True)
    st.download_button(
        "Download combined (CSV)",
        data=combined_raw.to_csv(index=False).encode("utf-8"),
        file_name=f"spend_this_combined_min_median_{years}y.csv",
        mime="text/csv"
    )
except Exception:
    pass
...
if median_withdrawal_s is not None:
    ...
    # ---- Global summary (Lump Sum + Annual Habit) ----
    if median_withdrawal_g is not None and len(raw_rows_habit) > 0:
        try:
            base_lump_sum_g_h = float(np.nanmax([row.get("Global Median Ending Value", np.nan) for row in raw_rows_habit]))
        except Exception:
            base_lump_sum_g_h = np.nan
        if np.isfinite(base_lump_sum_g_h):
            annual_income_g_h = (median_withdrawal_g * base_lump_sum_g_h) if median_withdrawal_g <= 1.0 else median_withdrawal_g
            total_income_g_h = annual_income_g_h * int(retirement_years)
            summary_g_h = pd.DataFrame([{
                "Number of Retirement Years": int(retirement_years),
                "Annual Income Stream (Historically)": annual_income_g_h,
                "Total Median (Historical) Retirement Income": total_income_g_h,
            }])
            summary_g_h_fmt = summary_g_h.copy()
            summary_g_h_fmt["Annual Income Stream (Historically)"] = summary_g_h_fmt["Annual Income Stream (Historically)"].map(lambda v: f"${v:,.0f}")
            summary_g_h_fmt["Total Median (Historical) Retirement Income"] = summary_g_h_fmt["Total Median (Historical) Retirement Income"].map(lambda v: f"${v:,.0f}")
            st.subheader("Median Withdrawal Summary — Global (Lump Sum + Annual Habit)")
            st.caption("Global portfolio; assumes **20 bps** annual expenses; base uses the **highest median ending value** across allocations with annual contributions; withdrawal rates assume a **60% Equity / 40% Fixed** portfolio through retirement.")
            _msg_g_h = (
                f"Spending ${thinking_spend:,.0f} instead of ${whatif_spend:,.0f} "
                f"cost you ${total_income_g_h:,.0f} in lifetime retirement income."
            )
            st.markdown(
                f"<div style='white-space: normal; word-break: normal; overflow-wrap: break-word;'><strong>{_msg_g_h}</strong></div>",
                unsafe_allow_html=True
            )

    # ---- SPX summary (Lump Sum + Annual Habit) ----
    if median_withdrawal_s is not None and len(raw_rows_habit) > 0:
        try:
            base_lump_sum_s_h = float(np.nanmax([row.get("SPX Median Ending Value", np.nan) for row in raw_rows_habit]))
        except Exception:
            base_lump_sum_s_h = np.nan
        if np.isfinite(base_lump_sum_s_h):
            annual_income_s_h = (median_withdrawal_s * base_lump_sum_s_h) if median_withdrawal_s <= 1.0 else median_withdrawal_s
            total_income_s_h = annual_income_s_h * int(retirement_years)
            summary_s_h = pd.DataFrame([{
                "Number of Retirement Years": int(retirement_years),
                "Annual Income Stream (Historically)": annual_income_s_h,
                "Total Median (Historical) Retirement Income": total_income_s_h,
            }])
            summary_s_h_fmt = summary_s_h.copy()
            summary_s_h_fmt["Annual Income Stream (Historically)"] = summary_s_h_fmt["Annual Income Stream (Historically)"].map(lambda v: f"${v:,.0f}")
            summary_s_h_fmt["Total Median (Historical) Retirement Income"] = summary_s_h_fmt["Total Median (Historical) Retirement Income"].map(lambda v: f"${v:,.0f}")
            st.subheader("Median Withdrawal Summary — S&P 500 (SPX) (Lump Sum + Annual Habit)")
            st.caption("SPX portfolio; assumes **5 bps** annual expenses; base uses the **highest median ending value** across allocations with annual contributions; withdrawal rates assume a **60% Equity / 40% Fixed** portfolio through retirement.")
            _msg_s_h = (
                f"Spending ${thinking_spend:,.0f} instead of ${whatif_spend:,.0f} "
                f"cost you ${total_income_s_h:,.0f} in lifetime retirement income."
            )
            st.markdown(
                f"<div style='white-space: normal; word-break: normal; overflow-wrap: break-word;'><strong>{_msg_s_h}</strong></div>",
                unsafe_allow_html=True
            )

    # ---- Final combined median withdrawal summary table ----
    try:
        combined_summary_rows = []
        # Lump Sum rows (if previously computed)
        if 'annual_income_g' in locals() and 'total_income_g' in locals():
            row = {
                "Portfolio": "Global",
                "Years": int(retirement_years),
                "Annual — Lump Sum": float(annual_income_g),
                "Total — Lump Sum": float(total_income_g),
            }
            if 'annual_income_g_h' in locals() and 'total_income_g_h' in locals():
                row.update({
                    "Annual — Lump + Habit": float(annual_income_g_h),
                    "Total — Lump + Habit": float(total_income_g_h),
                })
            combined_summary_rows.append(row)
        if 'annual_income_s' in locals() and 'total_income_s' in locals():
            row = {
                "Portfolio": "SPX",
                "Years": int(retirement_years),
                "Annual — Lump Sum": float(annual_income_s),
                "Total — Lump Sum": float(total_income_s),
            }
            if 'annual_income_s_h' in locals() and 'total_income_s_h' in locals():
                row.update({
                    "Annual — Lump + Habit": float(annual_income_s_h),
                    "Total — Lump + Habit": float(total_income_s_h),
                })
            combined_summary_rows.append(row)
        if combined_summary_rows:
            combined_summary_df = pd.DataFrame(combined_summary_rows)
            display_df = combined_summary_df.copy()
            for col in ["Annual — Lump Sum", "Total — Lump Sum", "Annual — Lump + Habit", "Total — Lump + Habit"]:
                if col in display_df.columns:
                    display_df[col] = display_df[col].map(lambda v: f"${v:,.0f}")
            st.subheader("Median Withdrawal Strategy — Combined (Lump Sum vs Lump + Annual Habit)")
            st.dataframe(display_df, use_container_width=True)
            st.download_button(
                "Download combined median withdrawal summary (CSV)",
                data=combined_summary_df.to_csv(index=False).encode("utf-8"),
                file_name=f"spend_this_combined_median_withdrawal_{years}y.csv",
                mime="text/csv"
            )
    except Exception:
        pass
