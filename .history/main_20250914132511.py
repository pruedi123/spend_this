            ...
            try:
                if isinstance(df_, pd.DataFrame) and not df_.empty:
                    ...
                    if not row.empty:
                        ...
                        ann_cols = [c for c<truncated__content/>
# Remove any unquoted raw CSS block that defines `.info-card`
import re as _re_cleanup
import sys as _sys_cleanup
if "__file__" in globals():
    _mainpy_path = __file__
else:
    _mainpy_path = None
try:
    # Remove any unquoted raw CSS block that defines `.info-card { ... }`
    # Only if not inside a triple-quoted string or st.markdown
    # This is a best-effort in-source edit for this session (idempotent if not present)
    import inspect as _inspect_cleanup
    _src_lines = []
    _fname = None
    try:
        _fname = _mainpy_path if _mainpy_path else _inspect_cleanup.getfile(lambda: None)
    ...
    for line in _src_lines:
        if not in_css and _re_cleanup.match(r"^\s*\.info-card\s*{", line):
            in_css = True
            continue
        if in_css:
            if _re_cleanup.match(r"^\s*}\s*$", line):
                in_css = False
            continue
        _out_lines.append(line)
    if _src_lines and _src_lines != _out_lines:
