...
median_withdrawal = _lookup_median_withdrawal(df_withdrawals, int(retirement_years))
if median_withdrawal is not None:
    # If the median looks like a rate (e.g., 0.035 = 3.5%), convert to dollars using the
    # best Global Median Ending Value across allocations; otherwise treat as dollars already.
    base_lump_sum = float(np.nanmax(raw_df["Global Median Ending Value"].values)) if not raw_df.empty else np.nan
    if median_withdrawal <= 1.0 and np.isfinite(base_lump_sum):
        annual_income = median_withdrawal * base_lump_sum
    else:
        annual_income = median_withdrawal
