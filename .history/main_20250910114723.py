...
if median_withdrawal_g is not None:
    ...
    st.subheader("Median Withdrawal Summary — Global")
    ...
    _msg_g = (
        f"By spending what you wanted (${thinking_spend:,.0f}) instead of being a bit more frugal "
        f"and spending (${whatif_spend:,.0f}), you cost yourself <strong>${total_income_g:,.0f}</strong> of total lifetime retirement income."
    )
    st.markdown(f"<div style='white-space: normal; word-break: normal; overflow-wrap: break-word;'>{_msg_g}</div>", unsafe_allow_html=True)
else:
...
if median_withdrawal_s is not None:
    ...
    st.subheader("Median Withdrawal Summary — S&P 500 (SPX)")
    ...
    _msg_s = (
        f"By spending what you wanted (${thinking_spend:,.0f}) instead of being a bit more frugal "
        f"and spending (${whatif_spend:,.0f}), you cost yourself <strong>${total_income_s:,.0f}</strong> of total lifetime retirement income."
    )
    st.markdown(f"<div style='white-space: normal; word-break: normal; overflow-wrap: break-word;'>{_msg_s}</div>", unsafe_allow_html=True)
else:
