...
try:
    ...
    grand_rows = [
        {
            ...
            "Annual Retirement Income — Grand Total": annual_lump_g + habits_total_g + auto_annual_g,
            "Total — Lump Sum": annual_lump_g * years_int,
            "Total — Habits Total": habits_total_g * years_int,
            "Total — Auto": auto_annual_g * years_int,
            "Total — Grand Total": (annual_lump_g + habits_total_g + auto_annual_g) * years_int,
        },
        {
            "Portfolio": "SPX",
            "Years": years_int,
            ...
            "Annual Retirement Income — Grand Total": annual_lump_s + habits_total_s + auto_annual_s,
            "Total — Lump Sum": annual_lump_s * years_int,
            "Total — Habits Total": habits_total_s * years_int,
            "Total — Auto": auto_annual_s * years_int,
            "Total — Grand Total": (annual_lump_s + habits_total_s + auto_annual_s) * years_int,
        },
    ]
    ...
    for col in [
        "Annual Retirement Income — Lump Sum", "Annual Retirement Income — Habits Total", "Annual Retirement Income — Auto", "Annual Retirement Income — Grand Total",
        "Total — Lump Sum", "Total — Habits Total", "Total — Auto", "Total — Grand Total",
    ]:
        grand_disp[col] = grand_disp[col].map(lambda v: f"${v:,.0f}")

    st.subheader("Median Withdrawal — Grand Total (Lump + All Annual Habits + Auto)")
    st.dataframe(grand_disp, use_container_width=True)
    st.download_button("Download median withdrawal — Grand Total (CSV)", data=grand_df.to_csv(index=False).encode("utf-8"), file_name=f"median_withdrawal_grand_total_{years}y.csv", mime="text/csv")
except Exception:
    pass