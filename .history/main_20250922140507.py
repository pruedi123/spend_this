...
if years > 0 and (float(non_price) > 0 or float(frugal_price) > 0):
    ...
    try:
        ...
        # Resolve path: try alongside this file first, then CWD as fallback
        ...
        for _p in _candidates:
            try:
                ...
                pass
        if _xls_path is None:
            raise FileNotFoundError("cpi_factors.xlsx not found next to main.py or in CWD")

        # Read preferred sheet, then fallback
        try:
        ...
        # Prefer explicit column name, else auto-detect something like 'increase_factor'
        if _col_name not in _df_inc.columns:
            for c in list(_df_inc.columns):
                ...
                if "increase" in s and "factor" in s:
                    ...
                    break
        if _col_name not in _df_inc.columns:
            raise KeyError("No 'increase_factor' column found in CPI sheet")

        inc_series = _pd.to_numeric(_df_inc[_col_name], errors="coerce")
        inc_arr = inc_series.astype(float).to_numpy()  # keep original indexing; do NOT dropna
        _have_inc = inc_arr.size > 0
        _inc_meta = f"path={_xls_path}, sheet={_inc_sheet}, col={_col_name}, n={inc_arr.size}"
    except Exception as _e:
        # Fallback: try CSV sheets if Excel engine (e.g., openpyxl) is unavailable
        ...
        try:
            ...
            if _csv_path is None:
                raise FileNotFoundError(f"Excel load failed ({_e}); no CSV fallback found (looked for increase_factors.csv or cpi_increase.csv)")
            _df_inc_csv = _pd.read_csv(_csv_path)
            # Find column similar to 'increase_factor'
            _col_name = None
            ...
            if _col_name is None:
                _col_name = "increase_factor"
            if _col_name not in _df_inc_csv.columns:
                raise KeyError("CSV fallback present, but no 'increase_factor' column found")
            inc_series = _pd.to_numeric(_df_inc_csv[_col_name], errors="coerce")
            inc_arr = inc_series.astype(float).to_numpy()
            _have_inc = inc_arr.size > 0
            _inc_meta = f"path={_csv_path}, sheet=CSV, col={_col_name}, n={inc_arr.size} (CSV fallback)"
        except Exception as _e2:
            _inc_meta = f"CPI load failed: {_e}; CSV fallback failed: {_e2}"
        ...
        if section_toggle("Audit — Auto CPI (per window)"):
            try:
                ...
                if sims_a.empty:
                    ...
                    # Show inflated sticker prices only at purchase years
                    yrs = np.arange(int(years))
                    non_buy = (yrs % int(max(1, non_replace)) == 0)
                    fr_buy  = (yrs % int(max(1, frugal_replace)) == 0)
                    non_sticker = np.where(non_buy, float(non_price) * lev, np.nan)
                    fr_sticker  = np.where(fr_buy,  float(frugal_price) * lev, np.nan)

                    audit_df = pd.DataFrame({
                        "Year": yrs,
                        "CPI level": lev,
                        "Non-frugal Sticker (inflated)": non_sticker,
                        "Frugal Sticker (inflated)": fr_sticker,
                    ...
                    for c in [
                        ...
                        "Non-frugal Payment ($/yr)", "Frugal Payment ($/yr)",
                        "Invested Difference ($/yr)",
                    ]:
