...
# ---------------------------
try:
    # Hover note explaining how summary values are derived
    ...
    _hover_text = (
        ...
        "At retirement, we assume a Global or SP500 strategy that is 60% Stocks / 40% Bonds and utilizes Monte Carlo simulation to adjust spending, and we run the results against historical returns for both Global and SP500 portfolios (60/40 allocation). From there, to determine the annual spending, we use the Median average annual spending result to get the spending percentage of a portfolio by multiplying that withdrawal percentage by the Median Ending Value from the investment account where the spending differences are invested. For example, if the Median Global ending value is $1,000,000 and the retirement period is 30 years, the withdrawal rate that represents the median average annual spending withdrawal is 8.6%. Therefore, historically speaking, the median average annual spending in a global portfolio of 60% stocks and 40% bonds would have been $86,000 per year, or $2,580,000 in lifetime (real) spending."
    )
    _hover_attr = _html.escape(_hover_text).replace("\n", "&#10;")
    st.markdown(
        f"<div style='margin: 0.5rem 0 0.25rem 0;'><span title=\"{_hover_attr}\">ℹ️ <u>How to interpret these values (hover)</u></span></div>",
        unsafe_allow_html=True
    )
