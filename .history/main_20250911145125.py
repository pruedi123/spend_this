    ...
    if annual_rows:
        ...
        try:
            ...
            for _, r in annual_df.iterrows():
                ...
                p = str(r.get("Portfolio", "")).strip().lower()
                portfolio_label = "the Global portfolio" if p == "global" else "the S&P 500 (SPX) portfolio"
                _msg_h = (f"Spending ${h['daily']:,.2f} instead of ${h['frugal']:,.2f} cost you ${total_val:,.0f} in lifetime retirement income by investing the differnce in spending amounts in {portfolio_label}.")
